                    ...
                    <div className="space-y-3">
                      {emotionalClusters.map((cluster) => (
                        <div key={cluster.id} className="space-y-1">
                          <div 
                            className={`flex items-center justify-between p-2 rounded cursor-pointer hover:bg-muted/50 ${selectedCluster === cluster.name ? 'bg-muted/50 ring-1 ring-primary/20' : ''}`}
                          ...
                          {clusterExpanded[cluster.name] && (
                            <div className="pl-5 pr-2 py-1 text-sm text-muted-foreground border-l-2 ml-1" style={{ borderColor: cluster.color }}>
                              <p className="mb-1 text-xs">Sentiment: {Math.round(cluster.sentiment * 100)}% {cluster.sentiment > 0.6 ? "Positive" : cluster.sentiment < 0.4 ? "Negative" : "Neutral"}</p>
                  ...
                  <CardContent>
                    {wordsForComparison.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-[200px] text-center p-4">
                        ...
                        <h3 className="text-lg font-medium mb-2">Compare Words</h3>
                        <p className="text-sm text-muted-foreground max-w-md">
                          Add words from the visualization to compare their emotional relationships and language patterns.
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        <div className="flex flex-wrap gap-2">
                          {wordsForComparison.map((point) => (
                            ...
                            >
                              {point.word}
                              <X
                                className="h-3 w-3 cursor-pointer hover:opacity-100 opacity-70"
                        ...
                        {wordsForComparison.length > 1 ? (
                          ...
                          <p className="text-sm text-muted-foreground italic text-center py-4">
                            Add at least one more word to see comparison
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    <Popover open={wordSearchOpen} onOpenChange={setWordSearchOpen}>
                      <PopoverContent 
                        className="p-0 w-64 max-h-[300px] overflow-y-auto"
                        ref={wordSearchRef}
                      >
                        <Command>
                          <CommandInput 
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {wellbeingSuggestions.map((suggestion, index) => (
                        <div key={index} className="p-3 rounded-lg border border-border bg-card/50 hover:bg-muted/30 transition-colors">
                    ...
                    <div className="mt-6">
                      <h3 className="font-medium mb-2 text-sm">Mental Health Resources</h3>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        {mentalHealthResources.map((resource, index) => (
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="p-2 rounded border border-border bg-card/50 hover:bg-muted/30 transition-colors text-sm"
                          >
